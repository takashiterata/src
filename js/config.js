...
async function FncListTable(PLUGIN_ID){
  try{
    ...
    for(let i=1;i<=tabini;i++){
      let ii=i-1;
      HtmlInnerVal += '<input type="text" id="aaButton' + i + '" class="tab-select2" value="'+ tabselect2val[ii] +'" onclick="FncTabonclick('+ i +')" style="width:96px;border-radius:10px 10px 0px 0px;background-color:#f5f5f5;padding: 1px 6px;text-align:center;" maxlength="20">';
    }
...
function FnccngTabini(e){
  ...
  let HtmlInnerVal ="";
  for(let i=1;i<=tabselectini.value;i++){
    HtmlInnerVal += '<input type="text" id="aaButton' + i + '" class="tab-select2" value="" onclick="FncTabonclick('+ i +')" style="width:96px;border-radius:10px 10px 0px 0px;padding: 1px 6px;text-align:center;" maxlength="20">';
  }
